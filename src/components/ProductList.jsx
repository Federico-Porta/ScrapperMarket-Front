...
import { useNavigate } from 'react-router-dom'; // Importante para el redireccionamiento
...
const ProductList = () => {
   ...
   const handleAddToCart = async (product) => {
       ...
       try {
           // AJUSTE AQUÍ: Cambiamos 'ean' por 'productEan'
           const cartRequest = {
               productEan: product.ean, // Nombre exacto que espera tu DTO
               quantity: 1
           };

           console.log("Enviando al carrito:", cartRequest);

           await axios.post('http://localhost:8080/cart/add', cartRequest, {
               headers: {
           ...
           // Esto te ayudará a ver si el error viene del Java
           alert("Error: " + (error.response?.data || "No se pudo añadir el producto"));
       }
   };
        ...
        <div style={{ padding: '20px' }}>
            <div style={filterContainer}>
                <input
                    ...
                    style={inputStyle}
                    onKeyPress={(e) => e.key === 'Enter' && (setCurrentPage(0), fetchProducts(0))}
                />

                <select
                    value={selectedStore}
                    onChange={(e) => { setSelectedStore(e.target.value); setCurrentPage(0); }}
                    style={selectStyle}
                >
                    <option value="">Todos los Supermercados</option>
                    {stores && stores.map(s => (
                        <option key={s.id} value={s.fantasyName}>{s.fantasyName}</option>
                    ))}
                </select>
                ...
                <>
                    ...
                    {totalPages > 1 && (
                        <div style={paginationContainer}>
                            <button disabled={currentPage === 0} onClick={() => setCurrentPage(prev => prev - 1)} style={currentPage === 0 ? disabledPageBtn : pageBtnStyle}>⬅ Anterior</button>
                            <span style={pageInfo}>Página <strong>{currentPage + 1}</strong> de <strong>{totalPages}</strong></span>
                            <button disabled={currentPage >= totalPages - 1} onClick={() => setCurrentPage(prev => prev + 1)} style={currentPage >= totalPages - 1 ? disabledPageBtn : pageBtnStyle}>Siguiente ➡</button>
                        </div>
                    )}
                </>
            )}
...
const imageContainer = { width: '100%', height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '10px' };
